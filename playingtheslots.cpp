...
typedef pair<string, double> command;
...
int main() {
    ...
    for (int i = 0; i < 1e4; i++) {
        ...
        double theta = i * 180/1e4 ;
        for (int j = 0; j < N; j++) {
            point new_p = rotate(init[j], theta);
            rotation.push_back(new_p);
        }
        std::sort(rotation.begin(), rotation.end());
        if (rotation[N-1].x - rotation[0].x < min)
            min =rotation[N-1].x - rotation[0].x;
    }
    cout<<min<<endl;
}